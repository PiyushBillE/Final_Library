...
import universityLogo from 'figma:asset/be269e711f7cbecfc3d5236bb30eb797aefa5e42.png';
...
export function StudentManagement({ user, onLogout, onNavigateBack }: StudentManagementProps) {
  ...
  const drawCardFront = async (doc: any, x: number, y: number, width: number, height: number, student: StudentData) => {
    ...
    // Header with university branding - updated text and no gradient
    const headerHeight = 20;
    doc.setFillColor(59, 130, 246); // Keep blue header, just remove gradient
    doc.rect(x, y, width, headerHeight, 'F');
    
    // Add university logo on the left side of header
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        logoImg.src = universityLogo;
      });
    } catch (error) {
      console.error('Error loading university logo:', error);
    }
    
    // University name in header - updated format (moved slightly right to accommodate logo)
    doc.setTextColor(255, 255, 255); // White text on blue background
    doc.setFontSize(8);
    doc.text('BHARATI VIDYAPEETH', x + width/2 + 10, y + 4, { align: 'center' });
    doc.setFontSize(7);
    doc.text('(Deemed to be University)', x + width/2 + 10, y + 8, { align: 'center' });
    doc.setFontSize(6);
    doc.text('DEPARTMENT OF ENGINEERING & TECHNOLOGY', x + width/2 + 10, y + 12, { align: 'center' });
    doc.setFontSize(7);
    doc.text('Library Card', x + width/2 + 10, y + 17, { align: 'center' });
    
    // Student photo area with better styling - portrait size photo
    const photoX = x + 8;
    const photoY = y + headerHeight + 5;
    const photoWidth = 24;
    const photoHeight = 30; // Portrait ratio photo (3:4 aspect ratio)
    ...
    // Student details - swapped Library Number and PRN positions
    doc.setFontSize(7);
    doc.setTextColor(75, 85, 99);
    
    const details = [
      `Library ID: ${student.libraryNumber}`, // Moved Library ID to original PRN position
      `Course: ${student.course}`,
      `Gender: ${student.gender}`,
      `Blood: ${student.bloodGroup}`
    ];
    
    // Calculate starting position to avoid footer overlap with reduced spacing
    const detailStartY = infoY + (nameLines.length * 4) + 6;
    details.forEach((detail, index) => {
      doc.text(detail, infoX, detailStartY + (index * 4));
    });
    
    // PRN number footer with barcode - changed to white background with blue border
    const footerY = y + height - 9; // Position for footer to avoid overlap with larger photo
    const footerHeight = 8;
    
    // White background with blue border
    doc.setFillColor(255, 255, 255);
    doc.rect(x + 3, footerY, width - 6, footerHeight, 'F');
    doc.setDrawColor(59, 130, 246);
    doc.setLineWidth(1);
    doc.rect(x + 3, footerY, width - 6, footerHeight);
    
    // PRN text
    doc.setTextColor(59, 130, 246);
    doc.setFontSize(9);
    doc.text(`PRN: ${student.prn}`, x + 6, footerY + 3);
    
    // Generate barcode for PRN
    try {
      const JsBarcode = await import('jsbarcode@3.11.6');
      
      // Create canvas for barcode
      const barcodeCanvas = document.createElement('canvas');
      JsBarcode.default(barcodeCanvas, student.prn, {
        format: "CODE128",
        width: 1,
        height: 25,
        displayValue: false,
      ...
      // Position barcode on the right side of footer
      const barcodeWidth = 32;
      const barcodeHeight = 5;
      const barcodeX = x + width - barcodeWidth - 6;
      const barcodeY = footerY + 1.2;
      
      doc.addImage(barcodeDataUrl, 'PNG', barcodeX, barcodeY, barcodeWidth, barcodeHeight);
      
    } catch (barcodeError) {
      console.error('Barcode generation error:', barcodeError);
      // Fallback: draw a simple rectangle pattern
      doc.setFillColor(59, 130, 246);
      const barcodeX = x + width - 32 - 6;
      const barcodeY = footerY + 1.2;
      for (let i = 0; i < 20; i++) {
        if (i % 2 === 0) {
          doc.rect(barcodeX + i * 1.5, barcodeY, 0.8, 5, 'F');
        }
      }
      ...
      doc.setFontSize(4);
      doc.text('Barcode', barcodeX + 13, barcodeY - 1, { align: 'center' });
    }
  ...
  const drawCardBack = async (doc: any, x: number, y: number, width: number, height: number, student: StudentData, QRCode: any) => {
    ...
    // Student details section - removed Category, Parent Mobile, and Blood Group
    doc.setTextColor(75, 85, 99);
    doc.setFontSize(7);
  ...
  const exportToExcel = async (studentsToExport: StudentData[]) => {
    try {
      ...
      // Create workbook and basic data worksheet
      const wb = XLSX.utils.book_new();
      
      // Prepare data with reordered columns including photo URL
      const excelData = studentsToExport.map(student => ({
        'Photo URL': student.photoUrl || 'No photo',
      ...
      const colWidths = [
        { wch: 80 }, // Photo URL - wider for URL
        { wch: 12 }, // Branch
        { wch: 15 }, // PRN
        { wch: 25 }, // Name
        { wch: 15 }, // Phone Number
      ...
      // Create a separate photo gallery worksheet
      try {
        await createPhotoGalleryWorksheet(wb, studentsToExport);
      } catch (photoError) {
        console.warn('Could not create photo gallery:', photoError);
      }
  ...
  // Helper function to create photo gallery worksheet
  const createPhotoGalleryWorksheet = async (workbook: any, students: StudentData[]) => {
    const XLSX = await import('xlsx');
    ...
    // Add header
    photoGalleryData.push(['Student Photo Gallery', '', '', '']);
    ...
    for (let i = 0; i < students.length; i++) {
      ...
      photoGalleryData.push([
        ...
        `PRN: ${student.prn}`,
        `Course: ${student.course}`,
        `Library ID: ${student.libraryNumber}`
      ]);
      
      // Add photo URL info
      photoGalleryData.push([
        'Photo URL:',
        student.photoUrl || 'No photo available',
        '',
        ''
      ]);
      
      // Add note about viewing photo
      photoGalleryData.push([
        'Note: Copy and paste the photo URL in browser to view the image',
        '',
        '',
        ''
      ]);
    ...
    // Create worksheet
    const photoWs = XLSX.utils.aoa_to_sheet(photoGalleryData);
    
    // Set column widths
    photoWs['!cols'] = [
      { wch: 30 }, // Student info
      { wch: 50 }, // Photo URL or additional info
      { wch: 20 }, // Course info
      { wch: 25 }  // Library ID
    ];
    
    // Set row heights
    if (!photoWs['!rows']) photoWs['!rows'] = [];
    for (let i = 0; i < photoGalleryData.length; i++) {
      photoWs['!rows'][i] = { hpt: 20 };
    }
    ...
    // Add the photo gallery worksheet
    XLSX.utils.book_append_sheet(workbook, photoWs, 'Photo Gallery');
  };

  const exportToCSV = (studentsToExport: StudentData[]) => {
    const headers = [
      'Photo URL', 'Branch', 'PRN', 'Name', 'Phone Number', 'Library Number', 'Email', 
